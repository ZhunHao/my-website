...
if (tagRefVersion && isSemver(tagRefVersion)) {
  ...
  console.log(
    `Version check passed: package.json=${pkgVersion} matches tag=${refName}.`,
  );
  process.exit(0);
}
...
let latestTag = "";
try {
  const out = execSync("git tag --list --sort=-v:refname", {
    encoding: "utf8",
  }).trim();
  latestTag = out.split("\n").find((t) => t.length > 0) || "";
} catch (err) {
  console.warn(
    "Could not read git tags; skipping non-tag version guard.",
    err.message,
  );
  process.exit(0);
}
