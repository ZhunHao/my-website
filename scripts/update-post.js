...
async function main() {
  try {
    ...
    if (
      ...
      if (options.imagePositionx)
        frontmatter.image.positionx = options.imagePositionx;
      if (options.imagePositiony)
        frontmatter.image.positiony = options.imagePositiony;
    }
    ...
    newFrontmatterStr = newFrontmatterStr.replace(
      /tags: ['"]?\[(.+?)\]['"]?/g,
      "tags: [$1]",
    );
